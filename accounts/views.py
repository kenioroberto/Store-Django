        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    # Produto variação e id no carrinho
                    product_variation = []
                    for item in cart_item:
                        variantion = item.variations.all()
                        product_variation.append(list(variantion))
                    
                    # itens do carrinho aceeso do usuario
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            messages.success(request,'Seu perfil foi editado!')
    ...
    if request.method =='POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(request, 'Senhas não válida')
                return redirect('change_password')
        else: 
